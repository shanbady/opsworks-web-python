...
script "set_release" do
  ...
  code <<-EOH
    ...
    if [ -z "$IMPACT_ENVIRONMENT" ]; then 
        ...
        .venv/local/bin/ecs deploy --ignore-warnings $IMPACT_ENVIRONMENT impact --image web $DOCKER_REGISTRY/impact-api:$DEPLOY_TARGET --image redis $DOCKER_REGISTRY/redis:$DEPLOY_TARGET --access-key-id $ECS_ACCESS_KEY_ID --secret-access-key $ECS_SECRET_ACCESS_KEY --region $AWS_DEFAULT_REGION --timeout 6000
     fi
  EOH
end
